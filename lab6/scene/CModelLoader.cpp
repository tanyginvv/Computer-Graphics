...
void CModelLoader::Load3dsFile(const char* fileName, CModel& model)
{
	...
	// содержащий 3ds файл
	std::string fileFolder = (slashPos == std::string::npos) ? "" : filePath.substr(0, slashPos + 1);
...
void CModelLoader::LoadMeshes(Lib3dsFile const& file, CModel& model)
{
	...
	const int materialCount = file.nmaterials;
	...
	for (int i = 0; i < meshCount; ++i)
	{
		...
		LoadMesh(materialCount, mesh, model, vertexBufferData, indexBufferData);
	}
	...
	std::vector<MeshFace>& outputFaces)
{
	...
	if (pInputTexCoords != NULL)
	{
		...
		TexturedVertex* outputVertices = reinterpret_cast<TexturedVertex*>(
...
unsigned CModelLoader::FillIndexBufferData(
	std::vector<MeshFace> const& faces, std::vector<std::vector<unsigned>> const& materialFaces,
	std::vector<unsigned char>& indexBufferData)
{
	// количество граней
	const int numberOfFaces = int(faces.size());
	...
	std::vector<unsigned char>& indexBufferData)
{
	...
	const unsigned int vertexBufferOffset = sizeof(unsigned char) * unsigned int(vertexBufferData.size());
